...
if [ "$(echo "$screens" | wc -l)" -eq 1 ]; then
    MONITOR=$primary polybar main 2>&1 | tee -a /tmp/polybar1.log &
else
    ...
    for m in $screens; do
        if [ $m = $primary ]; then
            MONITOR=$m polybar main 2>&1 | tee -a /tmp/polybar1.log &
        else
            MONITOR=$m polybar secondary 2>&1 | tee -a /tmp/polybar2.log &
        fi
    done
fi
